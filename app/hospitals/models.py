...
class HospitalVisit(models.Model):
    ...
    hospital_name = models.CharField(
        max_length=100,
        blank=True,
        help_text="병원 이름 (직접 입력)"
    )
    hospital_address = models.CharField(
        max_length=200,
        blank=True,
        help_text="병원 주소 (직접 입력)"
    )
    hospital_phone = models.CharField(
        max_length=20,
        blank=True,
        help_text="병원 전화번호 (직접 입력)"
    )
    ...
    def __str__(self):
        hospital_display = self.hospital.name if self.hospital else self.hospital_name
        return f"{self.pet.name} - {hospital_display} ({self.visit_date})"

    def get_hospital_name(self):
        """병원 이름 반환 (Hospital 또는 직접 입력)"""
        return self.hospital.name if self.hospital else self.hospital_name

    def get_hospital_address(self):
        """병원 주소 반환"""
        return self.hospital.address if self.hospital else self.hospital_address

    def get_hospital_phone(self):
        """병원 전화번호 반환"""
        return self.hospital.phone if self.hospital else self.hospital_phone
